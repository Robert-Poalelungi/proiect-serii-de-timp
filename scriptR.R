...
ggAcf(pret_petrol, lag.max = 48) + ggtitle("ACF – nivel")
ggAcf(diff(pret_petrol), lag.max = 48) + ggtitle("ACF – d(1)")
...
autoplot(forecast(sarima_model, h = 24)) +
...
ggplot(data.frame(t = time(randamente), vol = garch11@sigma.t),
       aes(t, vol)) + geom_line(color = "steelblue") +
...
autoplot(cbind(pret_petrol, curs_usd_eur)) +
